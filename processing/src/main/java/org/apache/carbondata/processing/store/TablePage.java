...
public class TablePage {
  ...
  TablePage(CarbonFactDataHandlerModel model, int pageSize) throws MemoryException {
    ...
    for (int i = 0; i < dimensionPages.length; i++) {
      ...
      switch (spec.getDimensionType()) {
        ...
        case DIRECT_DICTIONARY:
          page = ColumnPage.newPage(DataType.INT, pageSize);
          dimensionPages[i] = page;
          break;
        case PLAIN_VALUE:
    ...
    for (int i = 0; i < measurePages.length; i++) {
      DataType dataType = measureSpec[i].getDataType();
      ColumnPage page;
      if (dataType == DataType.DECIMAL) {
        page = ColumnPage.newDecimalPage(DataType.DECIMAL, pageSize,
            measureSpec[i].getScale(), measureSpec[i].getPrecision());
      } else {
        page = ColumnPage.newPage(dataType, pageSize);
      }
      measurePages[i] = page;
    }
